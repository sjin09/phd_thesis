...
\floatfoot{The graph begins with the current cost of CCS sequencing for a 30-fold sequence coverage of a human genome using the Revio instrument and the number of ZMWs in the last SMRTcell. \textbf{a)} Exponential decay in CCS sequencing cost with doubling in the number of ZMWs per SMRTcell. \textbf{b)} Exponential decay in CCS sequencing cost with doubling in both CCS read length and the number of ZMWs per SMRTcell.}
\end{figure}
...
\floatfoot{Red triangle indicates the DSB repair site. DSB can be repaired either as a gene conversion or as a crossover. Sister chromatids are not shown for simplification.}
\end{figure}
