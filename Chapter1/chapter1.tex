...
The Darwin Tree of Life (DToL) project is an ambitious project that aims to sequence and assemble 70,000 eukaryotic species in Britain and Island. The advent of long-read sequencing, Hi-C sequencing and genome mapping technologies enables the routine sequencing and de novo assembly of highly contiguous and complete reference genomes that was unimaginable with short-read sequencing and that was only possible through hierarchical shotgun BAC clone sequencing and assembly. The advent of these technologies creates a new environment where the cost of sequencing is no longer the bottleneck towards creating a reference genome of a species, and instead sample acquisition and library preparation becomes the bottleneck for construction of chromosome-length scaffolds of the reference genome.

\section{Thesis objectives}

In this thesis, we leverage the public datasets from the DToL project to study the germline and somatic mutational processes across the Tree of Life. In the process, we demonstrate that PacBio CCS reads have sufficient base accuracy for single molecule somatic mutation detection, develop a method to identify somatic mutations from bulk normal tissue, agnostic of clonality and species, and apply our method to eukaryotic species from the DToL project to study the germline and somatic mutational processes in species hitherto unexplored. 
