...
\floatfoot{The red line through the origin represents an ideal case where empirical BQ is identical to the BQ score assigned to each CCS base.}
\end{centering}
\end{figure}
...
\floatfoot{The median number of SBS identified per CCS read decreases with increases in the number of subreads per CCS read.}
\end{centering}
\end{figure}
...
\end{tabular}
\floatfoot{The mutation burden per cell is slightly inflated than expected as the calculation includes the number of false positive mutations.}
\end{table}
...
\floatfoot{Similar to KX004 mutational spectrum, CB001 mutational spectrum was previously generated from single-cell clone expansion and sequencing \cite{Mitchell2022-ry}. In the CB001, sample a greater number of somatic mutations are derived from spontaneous deamination of 5-methylcytosine to thymine.}
\end{figure}
...
\floatfoot{The mutational spectrum in the second row is derived from haplotype phased somatic mutations from CCS bases with BQ score equal or greater than 93. The mutational spectrum in the third row is derived from haplotype phased somatic mutations from CCS bases with BQ score equal or greater than 100. The red triangle indicates where the number of substitutions is above what is expected compared to the gold-standard mutational spectrum in the first row.}
\end{figure}
...
\floatfoot{The use of CCS bases with higher BQ does not recover the expected mutational spectrum from PD47269d sample.}
\end{figure}
